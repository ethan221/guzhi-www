...
class ControllerCommonFileManager extends Controller {
	...
	public function upload() {
		...
		if (isset($this->request->get['directory'])) {
			$directory = rtrim(DIR_IMAGE . 'catalog/' . str_replace(array('../', '..\\', '..'), '', $this->request->get['directory']), '/');
                                                      $ossdir = rtrim( 'catalog/' . str_replace(array('../', '..\\', '..'), '', $this->request->get['directory']), '/');
                                                      $ossdir .= '/';
		} else {
			$directory = DIR_IMAGE . 'catalog';
                                                      $ossdir = 'catalog';
		...
		if (!$json) {
                                                      ...
                                                      $filename = Utils::convertStrType($filename);
                                                      $filename = html_entity_decode($filename, ENT_QUOTES, 'UTF-8');
                                                      if (preg_match('/[^\w\d_ -]/si', $filename)) {
                                                                $this->load->library('pinyin');
                                                                $pinyin = new Pinyin();
                                                                $filename = $pinyin->getPinyin( $filename );
                                                      }
                                                      $object = ltrim($ossdir.$filename, '/');
                                                      $content = file_get_contents($this->request->files['file']['tmp_name']);
                                                      try{
                                                                //oss put
                                                                $ossapi = new Ossapi();
                                                                $ossapi->putObject($object, $content);
                                                                $ossapi->getObject($object);
                                                                move_uploaded_file($this->request->files['file']['tmp_name'], $directory . '/' . $filename);
                                                                $json['success'] = $this->language->get('text_uploaded');
                                                      }  catch (Exception $e){
                                                              $json['error'] = var_export($e->getMessage(), true);
                                                      }
		}
