		...
		foreach ($results as $result) {
			...
			$data['products'][] = array(
				...
				'edit'       => $this->url->link('catalog/product/edit', 'token=' . $this->session->data['token'] . '&product_id=' . $result['product_id'] . $url, true),
                                                                        'sku'       => $this->url->link('catalog/product/sku', 'token=' . $this->session->data['token'] . '&product_id=' . $result['product_id'] . $url, true),
			);
		}
		...
		$this->response->setOutput(json_encode($json));
                  ...
                  public function sku(){
                                ...
                                if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
                                        $post_data = $this->request->post['sku_id'];
                                        if($this->model_catalog_product->saveSkusn($product_id, $post_data)){
                                                $this->error['success'] = '更新sku成功';
                                        }
                                }
                                ...
                                if(!empty($product)){
                                        $data['text_list'] = $product['name']."：商品编号(sku)列表";
                                        $data['sku_data'] = $this->model_catalog_product->getSkusnByProductId($product_id);
                                }
                                $data['heading_title'] = $this->language->get('heading_title');
                                $data['header']    = $this->load->controller('common/header');
                                $data['column_left'] = $this->load->controller('common/column_left');
                                ...
                                $this->response->setOutput($this->load->view('catalog/product_sku', $data));
                  }
}