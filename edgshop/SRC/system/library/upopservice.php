...
if (function_exists("date_default_timezone_set")) {
    date_default_timezone_set(upopconfig::$timezone);
}
...
class upopservice
{
    ...
    function __construct($args, $service)
    {
        ...
        switch($service)
        {
            case upopconfig::FRONT_PAY:
                $trans_type = $args['transType'];
                if ($trans_type != upopconfig::CONSUME && $trans_type != upopconfig::PRE_AUTH) {
                    //前台交易仅支持 消费 和 预授权
                    throw new Exception("Bad trans_type for front_pay. Use back_pay instead");
                }
                $this->api_url = upopconfig::$front_pay_url;
                $this->args = array_merge(upopconfig::$pay_params_empty,
                                        upopconfig::$pay_params, $args);
                $param_check = upopconfig::$pay_params_check;
                break;

            case upopconfig::BACK_PAY:
                $this->api_url = upopconfig::$back_pay_url;
                $this->args = array_merge(upopconfig::$pay_params_empty,
                                        upopconfig::$pay_params, $args);
                $param_check = upopconfig::$pay_params_check;
            ...
            case upopconfig::QUERY:
                $this->api_url = upopconfig::$query_url;
                $args['version']    = upopconfig::$pay_params['version'];
                $args['charset']    = upopconfig::$pay_params['charset'];
                $args['merId']      = upopconfig::$pay_params['merId'];

                if (empty(upopconfig::$pay_params['merId']) &&
                    empty(upopconfig::$pay_params['acqCode']))
                {
                    throw new Exception('merId and acqCode can\'t be both empty');
                }

                //acqCode在QUERY请求中作为保留域存在
                if (!empty(upopconfig::$pay_params['acqCode'])) {
                    $acqCode = upopconfig::$pay_params['acqCode'];
                ...
                $this->args = $args;
                $param_check = upopconfig::$query_params_check;
                break;

            case upopconfig::RESPONSE:
                $arr_args = array();
                $arr_reserved = array();
                if (is_array($args)) {
                    $arr_args       = $args;
        ...
        $has_reserved = false;
        $arr_reserved = array();
        foreach (upopconfig::$mer_params_reserved as $key) {
        ...
        //signature
        $this->args['signature']    = self::sign($this->args, upopconfig::$sign_method);
        $this->args['signMethod']   = upopconfig::$sign_method;
    ...
    static function sign($params, $sign_method)
    {
        if (strtolower($sign_method) == "md5") {
            ...
            foreach ($params as $key => $val) {
                if (in_array($key, upopconfig::$sign_ignore_params)) {
                    continue;
                }
                $sign_str .= sprintf("%s=%s&", $key, $val);
            }
            return md5($sign_str . md5(upopconfig::$security_key));
        }
...
eot;
    ...
    function post() 
    {
        return self::curl_call($this->api_url, $this->args, true, array(
                        CURLOPT_SSL_VERIFYPEER  => upopconfig::VERIFY_HTTPS_CERT,
                        CURLOPT_SSL_VERIFYHOST  => upopconfig::VERIFY_HTTPS_CERT,
                    ));
    }
