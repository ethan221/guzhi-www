...
class WeiboConfig
{
        const WB_AKEY= '533573447';
        const WB_SKEY= '7b9c108db136e43eab65dfd10ebc12d8';
        const WB_CALLBACK_URL= 'http://www.edgteam.cn/account/login/snscallback?type=weibo';
}
...
class WeiboAuth {
	...
	function authorizeURL()    { return 'https://api.weibo.com/oauth2/authorize'; }
                ...
                function userinfoURL(){ return 'https://api.weibo.com/2/users/show.json'; }

	/**
	 * construct WeiboOAuth object
	 */
	function __construct($access_token = '', $refresh_token = '') {
                        	$this->client_id = WeiboConfig::WB_AKEY;
		$this->client_secret = WeiboConfig::WB_SKEY;
                		$this->access_token = $access_token;
		$this->refresh_token = $refresh_token;
                                    $this->redirect_url = WeiboConfig::WB_CALLBACK_URL;
	}
	...
	function getAccessToken( $type = 'code', $keys ) {
		...
		return $token;
                   ...
                   function getUserInfo(){
                                    $params = array('access_token' => $this->access_token, 'uid' => $this->openid);
                                    $response = $this->oAuthRequest($this->userinfoURL(), 'GET', $params);
		$userinfo = json_decode($response, true);
		if ( is_array($userinfo) && !isset($userinfo['error']) ) {
                                                      return $userinfo;
		} else {
			throw new OAuthException("get userinfo failed." . $userinfo['error']);
		}
                   }
