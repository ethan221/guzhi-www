...
class ModelAccountCustomer extends Model {
	public function addCustomer($data) {
		...
		$customer_group_info = $this->model_account_customer_group->getCustomerGroup($customer_group_id);
                                    ...
                                    $sql = "INSERT INTO " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$customer_group_id . "', store_id = '" . (int)$this->config->get('config_store_id') . "',";
                                    $sql .= "member_id = '" . $data['id']. "',";
                                    $sql .= "telephone = '" . $data['phone']."', ";
                                    $sql .= "fullname = '" . $data['nick']."', ";
                                    $sql .= "logo = '" . $data['logo']."', ";
                                    $sql .= "ip = '" . $this->db->escape($this->request->server['REMOTE_ADDR']) ."', status = '1',";
                                    $sql .= "approved = '" . (int)!$customer_group_info['approval'] . "', date_added = NOW(),";
	...
	public function getCustomer($customer_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE member_id = '" . $member_id . "'");
