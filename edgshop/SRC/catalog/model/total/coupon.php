...
class ModelTotalCoupon extends Model {
        ...
        public function getTotal($total, $order_data) {
                                    $coupon_data = $order_data['coupon_data'];
                                    $products = $order_data['products'];
		if (!empty($coupon_data) && !empty($products)) {
                                                $this->load->language('total/coupon');
                                                foreach ($coupon_data as $item){
                                                        $coupon_info = $this->getCoupon($item['coupon_id'], $products);
                                                        ...
                                                        if ($coupon_info) {
                                                                ...
                                                                if ($coupon_info['shipping'] && isset($order_data['shipping_method'])) {
                                                                        if (!empty($order_data['shipping']['tax_class_id'])) {
                                                                                $tax_rates = $this->tax->getRates($order_data['shipping_method']['cost'], $order_data['shipping']['tax_class_id']);
                                                                                foreach ($tax_rates as $tax_rate) {
                                                                                        if ($tax_rate['type'] == 'P') {
                                                                                                $total['taxes'][$tax_rate['tax_rate_id']] -= $tax_rate['amount'];
                                                                                        }
                                                                        ...
                                                                        $discount_total += $order_data['cost'];
                                                                }
                                                                ...
                                                                if ($discount_total > 0) {
                                                                        $total['totals'][] = array(
                                                                                ...
                                                                                'title'      => sprintf($this->language->get('text_coupon'), $coupon_info['name']),
        ...
        public function getCoupon($coupon_id, $products) {
		$status = true;
	...
	public function getCoupon_OLD($code) {
		$status = true;
	...
	public function getTotal_OLD($total) {
...
//			}
//		}
                                $preOrder = $this->cache->get('order_prepay_'.$order_info['order_id']);
                                if(!empty($preOrder)){
                                          $coupon_data = array();
                                          foreach($preOrder['totals'] as $totals){
                                          ...
                                          if (!empty($coupon_data)) {
                                                  foreach($coupon_data as $coupon_info){
                                                        $this->db->query("INSERT INTO `" . DB_PREFIX . "coupon_history` SET coupon_id = '" . (int)$coupon_info['coupon_id'] . "', order_id = '" . (int)$order_info['order_id'] . "', customer_id = '" . (int)$order_info['customer_id'] . "', amount = '" . (float)$order_total['value'] . "', date_added = NOW()");
                                                        $this->db->query("UPDATE `" . DB_PREFIX . "customer_coupon` SET order_id='". $order_info['order_id'] ."', coupon_status='1' WHERE customer_coupon_id='". (int)$coupon_info['customer_coupon_id'] ."'");
                                                  }
                                         } else {
                                                  return $this->config->get('config_fraud_status_id');
                                          }
	...
	public function unconfirm($order_id) {
		$this->db->query("DELETE FROM `" . DB_PREFIX . "coupon_history` WHERE order_id = '" . (int)$order_id . "'");
                                    $this->db->query("UPDATE `" . DB_PREFIX . "customer_coupon` SET order_id='0' AND coupon_status='0'  WHERE order_id='". $order_id['order_id'] ."' AND coupon_status='1'");
	}
}
