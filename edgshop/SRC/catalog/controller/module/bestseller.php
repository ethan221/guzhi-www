...
class ControllerModuleBestSeller extends Controller {
	public function index($setting) {
		...
		$results = $this->model_catalog_product->getBestSellerProducts($setting['limit']);
                                    if(empty($results) || count($results)<8){//暂时无订单
                                            $results = $this->model_catalog_product->getPopularProducts($setting['limit']);
                                    }
		if ($results) {
			foreach ($results as $result) {
				if ($result['image']) {
					$image = $this->model_tool_image->resize($result['image'], 281, 294);
				} else {
					$image = $this->model_tool_image->resize('placeholder.png', 281, 294);
				}
