...
//		 $customer = Customer::register($register_data);
		...
		echo 'Test End';
                  ...
                  public function osstest(){
                                //$ossapi = Ossapi::getInstance();
                                
                                $path ='D:\projects\edg-shop\SRC\image\catalog\duanxiu-yeguang';
                                $dir = new RecursiveDirectoryIterator($path);
                                print_r($this->get_files($dir));
               ...
               public function get_files($dir) {
                   $files = array();
                   $start = strlen('D:\projects\edg-shop\SRC\image/');
                   $this->load->library('pinyin');
                   $pinyin = new Pinyin();
                   $ossapi = new Ossapi();
                   for (; $dir->valid(); $dir->next()) {
                       if ($dir->isDir() && !$dir->isDot()) {
                           if($dir->getFilename() == 'cache')    continue;
                           if ($dir->haschildren()) {
                               $this->get_files($dir->getChildren());
                           };
                       }else if($dir->isFile()){
                               if(in_array($dir->getExtension(), array('jpg', 'png', 'gif', 'bmp'))){
                                        //$files[] = str_replace('\\', '/', substr( $pinyin->getPinyin(iconv("gbk","UTF-8", $dir->getPathName())), $start ));
                                        $object = str_replace('\\', '/', substr( $pinyin->getPinyin( iconv("gbk", "UTF-8", $dir->getPathName())), $start ) );
                                         echo "<hr />";
                                        echo $object;
                                       echo "<br />";
                                        $filename = html_entity_decode($dir->getPathname(), ENT_QUOTES, 'UTF-8');
                                        $content = file_get_contents(str_replace('\\', '/', $filename));
                                        try{
                                                $ossapi->putObject($object, $content);
                                        }  catch (Exception $e){
